module Treeview exposing (
  ...
  update, nodeKey, nodeChildren, setNodeChildren, view, nodeTitle,
...
-}
type Node = Node Key Title Options Children
...
type alias Icon = (String, String)
type alias Class = String
type alias Selectable = Bool
type alias Expand = Bool
type alias Children = Maybe (List Node)
...
update msg model =
  case msg of (Toggle key) -> toggle key model
              (Search val) -> search val model
              (ToggleCheck multiple cascade key value) -> check multiple cascade key value model
              _ -> model
  ...
  case c of
      Nothing ->
        let
          ...
          match = String.contains pattern title
          options = { opt | visible = match }
        in
...
viewItem_ config (Node key title opt children) =
  let
    ...
    (iconC, iconO) = Maybe.withDefault ("", "") icon
    cls = Maybe.withDefault "" class
    ic = if opt.opened then iconO else iconC
    cl = "item " ++ cls ++
...
awesome name = H.i [HA.class ("fa fa-" ++ name), HA.attribute "aria-hidden" "true"] []

-- find for List.
find : (a -> Bool) -> List a -> Maybe a
find predicate list = case list of
  [] -> Nothing
  first :: rest ->
...
optional cond elt = if cond then elt else H.text ""
